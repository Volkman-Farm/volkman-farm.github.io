---
title: Catching water where it falls
description: "Swales, basins, mulch, and roof water on a flat Florida lot with a high water table. The list of what we would build to keep our rain, and what we have built so far."
date: 2026-08-12T09:00:00-04:00
pillar: Permaculture in practice
hero: /assets/blog/catching-water-where-it-falls.svg
hero_alt: "Line drawing of a hillside cross section with a level swale ditch and berm, a mulched basin dished around a planted mound, and a barrel catching runoff from a shed roof"
...
The phrase people use is slow it, spread it, sink it. Water that is moving is water leaving, and every good idea here is a variation on making it stop and hang around long enough to soak in. What follows is roughly in order of cheapest to hardest, which is also the order we would do it in.

## Mulch, which we are already half doing by accident

The cheapest water catchment in the world is a layer of mulch, and we have some, and we did not put it down for this reason.

We laid three to four inches [to move the yard from ant country toward worm country](/blog/building-for-worms/). What we were not thinking about is that a mulch layer is also a sponge sitting on top of the ground. Rain hits it, gets held in it, and goes into the soil slowly over the following hours instead of arriving all at once and running for the low spot. It also stops the sun from cooking the moisture back out the next afternoon, which on a Florida August day is not a small thing.

So the first move is not clever. It is more mulch, over more of the yard, deeper. No design work, no digging, no permits, just the least interesting labor we own. It is a decent rule that the first thing on any list like this should be boring.

## Basins, which are just dents

Every one of our food forest mounds sits on high ground on purpose, because [that is what the contours told us to do](/blog/zones-for-2-17-acres/). Trees on the rises, water-tolerant things in the lows. That rule is still right and it has one obvious side effect: water does not stay where the fruit trees are. It leaves, which is exactly what we wanted for the roots and exactly what we did not want for the watering can.

A basin fixes that with a shovel. You dish out a shallow saucer on the uphill side of the mound, wide and only a few inches deep, and fill it with mulch. Rain running downhill hits the dish, stops, and sinks in over the root zone instead of continuing past it. It is not a pond and it is not supposed to hold water for days. It is supposed to hold it for twenty minutes, which is enough.

This is the item on the list we are most likely to do first, because a wheelbarrow and an afternoon covers it and the mounds are already there.

## Swales, which we are genuinely unsure about

A swale is a ditch dug dead level along the contour of a slope, with the dug-out soil mounded on the downhill side. Water running downhill hits the ditch, cannot go anywhere because there is no downhill left inside it, and sits there soaking in along its whole length. On a hillside it is the best trick in the book.
...
## Roof water, which is the embarrassing one
