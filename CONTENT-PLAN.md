...
| Date | Pillar | Working title | Angle | Status |
|---|---|---|---|---|
| Sun Sep 6 | Variety spotlight | OPEN, first refusal to radish | **Mustard moved off this row to Aug 16 on 2026-08-19**, because radish was still out of rotation and Aug 16 needed a spotlight that was actually on the shelves. Give this slot to the finished radish draft in the backlog **if radish is back in rotation by then**; ask Albert first, do not assume. The backlog entry lists the two things to re-check before it runs. If radish is still out, the unspotlighted varieties left are kohlrabi (Aug 30) and chard (Sep 13), so pull one forward and reopen the row it came from rather than writing a second post on a variety that already has one. | [ ] |
